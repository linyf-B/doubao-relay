        ...
        const parser = createParser((event) => {
            try {
                ...
                if (rawResult.event_type == 2005) {
                    const ed = _.attempt(() =>
                        typeof rawResult.event_data === "string"
                            ? JSON.parse(rawResult.event_data)
                            : rawResult.event_data
                    );
                    const code = !_.isError(ed) ? ed?.code || ed?.error_detail?.code : null;
                    const msg = !_.isError(ed)
                        ? ed?.message || ed?.error_detail?.message || JSON.stringify(ed)
                        : String(rawResult.event_data);
                    if (code === 710022004 || /rate\s*limited/i.test(String(msg))) {
                        throw new APIException(EX.API_REQUEST_FAILED, `豆包限流/风控: ${msg}`);
                    }
                    throw new APIException(EX.API_REQUEST_FAILED, `豆包错误事件: ${msg}`);
                }
                ...
                if (result.code === 710022004 || /rate\s*limited/i.test(String(result.message || ""))) {
                    throw new APIException(EX.API_REQUEST_FAILED, `豆包限流/风控: ${result.message || result.code}`);
                }
                if (result.is_finish) {
                    isEnd = true;
                    finalize();
                ...
                // 新版 2010
                if (ctype === 2010 && !_.isError(parsed) && Array.isArray(parsed?.data)) {
                    parsed.data.forEach((item: any) => {
                        const ori = item?.image_ori?.url || item?.image_raw?.url || item?.image_preview?.url || item?.image_thumb?.url || item?.url;
                        const key = item?.key || ori;
                        if (ori && key && !emittedImageKeys.has(key)) {
                            emittedImageKeys.add(key);
                            imageUrls.push(String(ori));
                        }
                    });
                }
                if (ctype === 2074) {
                    ...
                    if (!_.isError(payload) && payload && Array.isArray(payload.creations)) {
                        payload.creations.forEach((c: any) => {
                            const img = c?.image || {};
                            const key = (img?.key || img?.image_ori?.url || img?.image_preview?.url || c?.key) as string | undefined;
                            const ori =
                                img?.image_ori?.url ||
                                img?.image_raw?.url ||
                                img?.image_thumb?.url ||
                                ...
                                img?.url ||
                                c?.url;
                            if (ori && /^https?:\/\//.test(String(ori))) {
                                const dedupe = key || ori;
                                if (!emittedImageKeys.has(dedupe)) {
                                    emittedImageKeys.add(dedupe);
                                    imageUrls.push(String(ori));
                                }
                            }
                        });
                ...
                // 兜底：文本里直接带图片链接
                if (typeof text === "string" && /https?:\/\/[^\s]+\.(?:png|jpe?g|webp)/i.test(text)) {
                    const matches = text.match(/https?:\/\/[^\s)"']+/g) || [];
                    for (const u of matches) {
                        if (/(\.png|\.jpe?g|\.webp|byteimg|imagex)/i.test(u) && !emittedImageKeys.has(u)) {
                            emittedImageKeys.add(u);
                            imageUrls.push(u);
                        }
                    }
                }
